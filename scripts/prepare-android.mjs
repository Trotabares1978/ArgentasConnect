...
fs.copyFileSync(
  native,
  path.join(targetDir, 'ArgentasSyncPlugin.java')
);
...
// Permisos necesarios para la sincronización local.
// Se agregan como hijos de <manifest>, antes de <application>.
...
if (fs.existsSync(manifest)) {
  ...
  if (missing.length) {
    ...
    if (!applicationTag.test(s)) {
      throw new Error(
        'No se encontró la etiqueta <application> en AndroidManifest.xml'
      );
    }
